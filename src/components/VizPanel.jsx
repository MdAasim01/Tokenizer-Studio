const KIND_STYLE = {
  ...
  space: "bg-zinc-100 text-zinc-600 border-zinc-300",
  special: "bg-amber-100 text-amber-800 border-amber-300",
};

export default function VizPanel({ words, hoverIndex, setHoverIndex, pinnedIndex, setPinnedIndex }) {
  return (
    <div className="border border-edge rounded-xl bg-panel">
      <div className="px-4 py-3 border-b border-edge flex items-center justify-between">
        <div className="font-semibold text-ink">Token Visualization</div>
      ...
      <div className="p-4 flex flex-wrap gap-2">
        {words.map((w) => {
          ...
          const cl =
            ...
            `${isPinned ? "ring-2 ring-warn" : isHover ? "ring-2 ring-accent2" : ""}`;
        ...
        {words.length === 0 && <div className="text-zinc-500 text-sm">Nothing to visualize.</div>}
      </div>
      ...
      <div className="px-4 pb-4">
        <div className="flex items-center gap-3 text-xs">
          ...
          <Legend color="bg-zinc-400" label="Whitespace" />
        </div>
      </div>
    </div>
  );
