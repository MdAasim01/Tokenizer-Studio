  ...
  return (
		<div className="border border-edge rounded-xl bg-panel dark:bg-slate-800 dark:border-slate-700">
			...
			<div className="p-4 grid gap-3">
				<label className="text-sm text-zinc-600 dark:text-slate-300">
					Enter IDs (CSV, space-separated, or JSON array)
				</label>
				<textarea
					className="w-full h-28 rounded-xl p-3 font-mono bg-white border border-edge dark:bg-slate-900 dark:border-slate-700 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-accent"
					value={idsQuery}
					onChange={(e) => setIdsQuery(e.target.value)}
					placeholder="Example: 158, 5, 199, 83 or [158,5,199,83]"
				/>
				<div>
					<div className="text-xs text-zinc-500 dark:text-slate-300 mb-1">
						Decoded Text
					</div>
					<div className="min-h-[2rem] font-mono rounded-xl p-3 bg-white border border-edge dark:bg-slate-900 dark:border-slate-700 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-accent">
