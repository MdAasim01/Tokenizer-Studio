...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-white text-ink">
      <header className="border-b border-edge bg-white sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-5 py-4 flex items-center justify-between">
          <h1 className="text-xl font-extrabold tracking-tight">
            Tokenizer <span className="text-accent">Studio</span>
          </h1>
          <div className="text-xs text-zinc-500">
            {encoded.stats.ids} IDs • {encoded.stats.words} tokens • {encoded.stats.chars} chars
          </div>
        </div>
      </header>

      <main className="max-w-7xl mx-auto px-5 py-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
        <section className="space-y-4">
          <div className="border border-edge rounded-xl bg-panel">
            <div className="p-4 border-b border-edge">
              <div className="text-sm text-zinc-600 mb-2">Suggestions</div>
              <SuggestionChips onPick={(t) => setText(t)} />
            </div>

            <div className="p-4">
              <label htmlFor="text" className="text-sm text-zinc-600">Text Input</label>
              <textarea
                id="text"
                className="mt-2 w-full h-40 bg-white border border-edge rounded-xl p-3 font-mono focus:outline-none focus:ring-2 focus:ring-accent"
                value={text}
              ...
              <div className="mt-3 flex items-center gap-2">
                <CopyButton
                  payload={text}
                  label="Copy Original"
                  onCopied={() => setCopiedBanner("Original copied")}
                />
                <button
                  className="text-sm px-3 py-1 rounded-lg border border-edge hover:bg-edge/40 cursor-pointer"
                  onClick={() => { setText(""); setPinnedIndex(null); setHoverIndex(null); }}
                >
                  Clear
                </button>
              </div>
